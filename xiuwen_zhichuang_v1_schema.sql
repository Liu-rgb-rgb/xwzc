...
INSERT INTO `shop_info` (`owner_user_id`, `shop_name`, `slogan`, `contact_name`, `contact_phone`, `email`, `address`, `description`, `status`, `deleted`)
VALUES (2, '测试商铺', '测试用店铺', '测试商家', '13900000001', 'test@example.com', '广东省广州市', '测试商家专用店铺', 1, 0);
...
-- ============================================================
-- 测试数据（消息通知），2026-08-03
-- 用于用户端-4. 消息 + 商家端-14. 消息管理接口调试：
--   [4.1] GET    /api/messages                      消息列表
--   [4.2] GET    /api/messages/unread-count          未读消息数量
--   [4.3] PUT    /api/messages/{messageId}/read      标记单条已读
--   [4.4] PUT    /api/messages/read-all              全部标记已读
--   [4.5] DELETE /api/messages/{messageId}           删除消息
--   [14.1] POST  /api/admin/messages                 发布消息通知
-- 场景覆盖：全局消息/个人消息、已读/未读、多种通知类型、跨用户隔离
-- ============================================================
INSERT INTO `message_notice` (`user_id`, `title`, `content`, `notice_type`, `is_read`, `deleted`)
VALUES
(NULL, '系统维护通知', '平台将于 2026-08-05 凌晨 2:00-4:00 进行系统升级维护，届时服务将暂时不可用。', 'SYSTEM', 0, 0),
(NULL, '新功能上线', 'AI 智能配色功能已上线，前往纹样创作页面立即体验。', 'SYSTEM', 0, 0),
(3,    '订单已发货',  '您的订单 #2026001（帆布袋定制款）已发货，预计 3 天内送达。', 'ORDER',    0, 0),
(3,    '订单已完成',  '您的订单 #2026002（丝巾礼盒）已确认收货，感谢您的购买。',  'ORDER',    1, 0),
