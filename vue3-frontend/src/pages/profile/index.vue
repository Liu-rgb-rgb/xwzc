...
async function confirmDeleteOrder() {
  ...
  error.value = '';
  try {
    if (!String(order.id).startsWith('local-')) await api.orders.remove(order.id);
    orders.value = orders.value.filter((item) => String(item.id || item.orderNo) !== key);
    writeUserData(
      'user_orders',
      readUserData<any[]>('user_orders', []).filter((item: any) => String(item.id || item.orderNo) !== key)
    );
    stats.value.orders = Math.max(0, stats.value.orders - 1);
    notice.value = '订单已删除';
    deletingOrder.value = null;
    activeOrderTab.value = 'all';
  } catch (reason: any) {
    error.value = reason?.response?.data?.message || '删除订单失败，请稍后重试';
  } finally {
    processingOrderId.value = null;
  }
}
...
<template>
  ...
  <section class="profile-page">
    ...
    <div class="profile-layout">
      ...
      <main class="profile-main">
        ...
        <section v-if="activeMenu === 'profile'" class="content-card">
          <div class="card-title"><div><span class="eyebrow">PROFILE</span><h2>完善头像与个人资料</h2></div><span class="last-login">上次登录：{{ formatTime(profile.lastLoginAt) || '暂无记录' }}</span></div>
        ...
        <section v-else-if="activeMenu === 'security'" class="content-card">
          <div class="card-title"><div><span class="eyebrow">ACCOUNT SECURITY</span><h2>账号安全</h2></div><span class="last-login">上次登录：{{ formatTime(profile.lastLoginAt) || '暂无记录' }}</span></div>
          <div class="security-page">
            <div class="security-card">
              ...
              <div class="security-row"><span>登录安全<small>{{ profile.lastLoginAt ? `上次登录 ${formatTime(profile.lastLoginAt)}` : '建议定期修改密码' }}</small></span><button @click="openPasswordEdit">›</button></div>
            </div>
          </div>
        </section>
        ...
        <section v-else-if="activeMenu === 'orders'" class="content-card">
          ...
          <div class="profile-order-list">
            <article v-for="order in filteredOrders" :key="order.id || order.orderNo" class="profile-order-card">
              <div class="order-head"><div><b>订单号：{{ order.orderNo || order.id }}</b><small>{{ formatTime(order.createdAt || order.createTime) }}</small></div><i :class="{ failed: rawOrderStatus(order) === 'CANCELLED' }">{{ orderStatusLabel(order) }}</i></div>
              ...
              <div class="order-footer">
                ...
                <div v-else-if="rawOrderStatus(order) === 'CANCELLED' || rawOrderStatus(order) === 'COMPLETED'" class="order-actions">
                  <button type="button" :disabled="Boolean(processingOrderId)" @click="deletingOrder = order">{{ processingOrderId === String(order.id || order.orderNo) ? '处理中…' : '删除订单' }}</button>
                </div>
              </div>
            </article>
    ...
    <!-- 删除订单确认弹窗 -->
    <div v-if="deletingOrder" class="modal-backdrop" role="presentation" @click.self="deletingOrder = null">
      <section class="cancel-order-dialog" role="alertdialog" aria-modal="true" aria-label="删除订单">
        <span class="dialog-icon">?</span>
        <h3>删除该订单？</h3>
        <p>订单 {{ deletingOrder.orderNo || deletingOrder.id }} 将从订单列表中移除，删除后无法恢复。</p>
        <div class="cancel-order-actions">
          <button type="button" :disabled="Boolean(processingOrderId)" @click="deletingOrder = null">保留订单</button>
          <button class="primary" type="button" :disabled="Boolean(processingOrderId)" @click="confirmDeleteOrder">{{ processingOrderId ? '处理中…' : '删除订单' }}</button>
        </div>
      </section>
    </div>
    <!-- 密码修改结果弹窗 -->
