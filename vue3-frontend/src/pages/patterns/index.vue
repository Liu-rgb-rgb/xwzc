...
async function loadPatterns() {
  ...
  try {
    ...
    const localItems = localPatternLibrary();
    libraryItems.value = dedupePatterns(
      localItems.length ? localItems : demoPatterns.map(normalize)
    );
    items.value = itemsForCurrentTab(libraryItems.value);
...
async function toggleFavorite(item: PatternItem) {
  ...
  try {
    ...
    showNotice(
      item.favorite ? '后端暂不可用，收藏已同步到本地纹样库' : '后端暂不可用，本地收藏已取消'
    );
  }
}
...
async function applyToProduct(item: PatternItem) {
  try {
    ...
    showNotice('该纹样尚未成功保存，请重新生成后再应用到商品');
  }
}
      ...
      <section class="patterns-main">
        ...
        <div class="pagination">
          ...
          >
            ...
            <option :value="20">20 条/页</option></select
          ><label
            >跳至
            <input
              type="number"
