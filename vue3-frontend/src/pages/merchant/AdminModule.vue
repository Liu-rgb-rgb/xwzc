...
function rowStatus(row: any) {
  ...
  if (moduleName.value === 'products' && status)
    return productStatusLabels[String(status)] || status;
  if (moduleName.value === 'orders' && status) return orderStatusLabels[String(status)] || status;
  return status || (row.isRead === 1 ? '已读' : row.isRead === 0 ? '未读' : '正常');
}
function rowTime(row: any) {
  return row.updatedAt || row.updateTime || row.createdAt || row.createTime || '-';
}
